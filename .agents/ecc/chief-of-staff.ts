...
const definition: AgentDefinition = {
  ...
  toolNames: ["set_output", "read_files", "code_search", "glob", "run_terminal_command", "str_replace", "write_file"],
...
- **Rules are system-injected**: \`.freebuff/rules/*.md\` files load automatically every session. Unlike prompt instructions, the LLM cannot choose to ignore them.

## Example Invocations

\`\`\`bash
@agent mail                    # Email-only triage
@agent slack                   # Slack-only triage
@agent today                   # All channels + calendar + todo
@agent schedule-reply "Reply to Sarah about the board meeting"
\`\`\`

## Prerequisites

- [Freebuff](https://freebuff.com/docs)
- Gmail CLI (e.g., gog by @pterm)
- Node.js 18+ (for calendar-suggest.js)
- Optional: Slack MCP server, Matrix bridge (LINE), Chrome + Playwright (Messenger)`,
  spawnerPrompt: '--- name: chief-of-staff description: Personal communication chief of staff that triages email, Slack, LINE, and Messenger. Classifies messages into 4 tiers (skip/info_only/meeting_info/action_required), generates draft replies, and enforces post-send follow-through via hooks. Use when managing multi-channel communication workflows.',
