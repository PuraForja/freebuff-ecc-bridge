...
rsync -av --exclude='.git' --exclude='node_modules' --exclude='__pycache__' \
  ...
  --exclude='.freebuff/' --exclude='.secrets/' --exclude='secrets/' \
...
- Do not modify source code logic — only configuration and references`,
  spawnerPrompt: '--- name: opensource-forker description: Fork any project for open-sourcing. Copies files, strips secrets and credentials (20+ patterns), replaces internal references with placeholders, generates .env.example, and cleans git history. First stage of the opensource-pipeline skill.',
